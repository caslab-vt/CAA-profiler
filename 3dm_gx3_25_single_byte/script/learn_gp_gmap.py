...
for j in range(len(altitude)):
	if (X[j,0]- X[j,1])<0.0001:
		lat_data=(X[j,0]+ X[j,1])/2
		lon_data=(X[j,0]+ X[j,1])/2
		util_data=(X[j,0]+ X[j,1])/2
...
gmap3 = gmplot.GoogleMapPlotter(lats[0],lons[0],40)

# scatter method of map object  
# scatter points on the google map 

#	plt.scatter( lats[i], lons[i], c= color, s=altitude[i]*10, alpha=0.5 ) 
#plt.scatter(lats, lons, s=altitude, c=color, alpha=0.5)	
#gmap3.scatter( lats, lons, hex(color), size=20, marker= True ) 
# Plot method Draw a line in 
# between given coordinates 
#gmap3.plot(lats, lons, 'cornflowerblue', edge_width = 2.5) 
gmap3.circle(lats, lons, radius=altitude*10)


gmap3.apikey="AIzaSyANeVkUuKIp9QPNrnJ9vo_de5IZZZ7MC44"  
gmap3.draw("mymap.html")
#gmap3.draw( "map123.html" ) 
