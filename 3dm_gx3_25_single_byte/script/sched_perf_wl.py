...
def log_loads1(time,gps):
    ...
    ctx_swt=0
    #print os.getcwd()
    with open('/home/nvidia/catkin_ws/schedrr1_max_wl_map.txt','a') as f:
  	p = psutil.Process(loam_map_pid)
  	times=p.cpu_times()
...
def log_loads2(time,gps):
    ...
    global pr2
    #print os.getcwd()
    with open('/home/nvidia/catkin_ws/schedrr1_max_wl_rrt.txt','a') as f:
...
def log_loads3(time,gps):
    ...
    global pr3
    #print os.getcwd()
    with open('/home/nvidia/catkin_ws/schedrr1_max_wl_ta.txt','a') as f:
...
def log_loads4(time,gps):
    ...
    global time_usr4	
    #print os.getcwd()
    with open('/home/nvidia/catkin_ws/schedrr1_max_wl_odom.txt','a') as f:
  	p = psutil.Process(loam_odom_pid)
  	times=p.cpu_times()
#	mem = p.virtual_memory()
        gps = ','.join(map(str,gps))
        time = str(time)
...
#        data = list([time,gps,str(p.cpu_percent(interval=1)),str(100)])
#        data = list([time,gps,str(times[0]-time_usr),str(times[1]-time_sys),str(ang_vel), str(lin_acc),str(cpu_p),str(100)])
	data = list([time,gps,str(times[0]-time_usr4),str(times[1]-time_sys4),str(int(pr1)), str(100)])
        print(data)
        f.write(','.join(data))
	f.write('\n')
	time_usr4=times[0]
...
#    with open('/home/nvidia/catkin_ws/output_slam_maxff2.txt','a') as outfile:
	...
	if (i % 50)==0:
	    ...
	    cmd11="sudo chrt -r -p " + str(int(pr1)) + " " + str(loam_map_pid)
	    cmd12="sudo chrt -r -p " + str(int(pr1)) + " " + str(loam_odom_pid)
	    cmd13="sudo chrt -r -p " + str(int(pr1)) + " " + str(loam_trm_pid)
	    cmd14="sudo chrt -r -p " + str(int(pr1)) + " " + str(loam_msr_pid)
